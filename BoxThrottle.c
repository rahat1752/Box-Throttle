...
void intro() {
	...
	printf(CLI_RESET"First player is " CLI_BOLD_BLUE "X" CLI_RESET CLI_BOLD " and the second player is " CLI_BOLD_RED "O" CLI_RESET CLI_BOLD ".\n");
	printf("When a player enters their symbol on a cell, the adjacent cells are blocked, preventing the other player from playing their move on those cells.\n");
...
void user_move(BoardState **board) {
	...
	while (1)
	{
		...
		{
			print_board(board);
			printf(CLI_BOLD_RED"\t\t\tYou are trying to access blocked square\n");
			printf(CLI_RESET);
			continue;
		}
